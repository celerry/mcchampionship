...
async function getCurrentEventCycle(): Promise<EventInformationResponse> {
    const response = await fetch("https://api.mcchampionship.com/v1/event", { headers: { "Accept": "application/json" } })
    ...
    if(response.status === 200) {
        ...
        throw new Error(`Failed to get current event: got status code ${response.status}`)
    }
}
